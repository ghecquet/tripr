...
var fs = afero.NewBasePathFs(afero.NewOsFs(), "/")
...
func testFastWalk(t *testing.T, files map[string]string, callback func(path string, typ os.FileMode) error, want map[string]os.FileMode) {
	tempdir, err := afero.TempDir(fs, "", "test-fast-walk")
	if err != nil {
		t.Fatal(err)
	}
	...
	for path, contents := range files {
		...
		if strings.HasPrefix(contents, "LINK:") {
			...
			err = afero.WriteFile(fs, file, []byte(contents), 0644)
		}
		if err != nil {
			t.Fatal(err)
		}
	...
	var mu sync.Mutex
	if err := fastwalk.Walk(fs, tempdir, func(path string, typ os.FileMode) error {
		mu.Lock()
		defer mu.Unlock()
...
func BenchmarkFastWalk(b *testing.B) {
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		err := fastwalk.Walk(fs, *benchDir, func(path string, typ os.FileMode) error { return nil })
		if err != nil {
			b.Fatal(err)
		}
	}
